...
# Raw CSV content provided by user
...
def import_data():
    ...
    lines = STOPS_CSV_CONTENT.strip().split('\n')
    ...
    for row in reader:
        ...
        # Check if stop name exists in DB or dict
        bus_stop, created = BusStop.objects.get_or_create(
            ...
            defaults={
                ...
                'latitude': lat,
                'longitude': lon,
                'description': f"NWKRTC / BRTS Bus Stop ({row['stop_id']})"
            }
        )
        if not created:
            # Update lat/lon if needed
            bus_stop.latitude = lat
            bus_stop.longitude = lon
            bus_stop.save()
            
        stops_by_name[name.lower()] = bus_stop
        stop_count += 1

    print(f"Successfully processed {stop_count} Bus Stops!")
