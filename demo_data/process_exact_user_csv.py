...
raw_user_csv_text = None

with open(transcript_path, 'r', encoding='utf-8') as f:
    text = f.read()

# Find the last occurrence of 'bus_no,route_name,stops_name_between_route'
pos = text.rfind('bus_no,route_name,stops_name_between_route')
print(f"rfind position of header: {pos}")
...
if pos != -1:
    ...
    # stop at closing tag if present
    end_tag = snippet.find('</USER_REQUEST>')
    if end_tag != -1:
        snippet = snippet[:end_tag]
        
    lines = [l.strip() for l in snippet.splitlines() if l.strip() and ',' in l]
    raw_user_csv_text = '\n'.join(lines)
    print(f"Extracted {len(lines)} CSV lines!")

if not raw_user_csv_text:
    print("ERROR: Could not find user uploaded CSV content in transcript!")
    sys.exit(1)

# Save exact CSV file
csv_file_path = 'demo_data/user_uploaded_routes.csv'
with open(csv_file_path, 'w', encoding='utf-8') as out:
    out.write(raw_user_csv_text)
...
reader = csv.DictReader(io.StringIO(raw_user_csv_text))
...
# Reset DB
print("--- Deleting existing Bus, RouteStop, Route, BusLocation records ---")
...
# Import every single unique route
processed_routes_count = 0
processed_buses_count = 0
...
# Validation & Summary
print("==========================================")
print("FINAL SUMMARY & DATA INTEGRITY CHECK:")
...
if db_route_count == total_unique_routes_found:
    print("\nSUCCESS CRITERIA MET: 100% Data Integrity! Every unique route from the CSV is loaded into the DB.")
else:
    print(f"\nFAILURE: Route count discrepancy! Expected {total_unique_routes_found}, got {db_route_count} in DB.")
print("==========================================")
