...
**Answer**:
Leaflet.js is an open-source JavaScript mapping library. It attaches to an HTML `<div>` container (e.g. `<div id="map"></div>`) and fetches map tiles asynchronously using high-performance OpenStreetMap tile servers (`https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png`).
